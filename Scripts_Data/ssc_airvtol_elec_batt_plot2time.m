...
if ~exist('h2_ssc_airvtol_batt', 'var') || ...
        ~isgraphics(h2_ssc_airvtol_batt, 'figure')
    h2_ssc_airvtol_batt = figure('Name', 'Battery States in ssc_airvtol_elec_battery');
end
figure(h2_ssc_airvtol_batt)
clf(h2_ssc_airvtol_batt)
...
vtol_batt_C  = logsout_ssc_airvtol_elec.get('Battery Charge (SOC)');
vtol_batt_A  = logsout_ssc_airvtol_elec.get('Battery Current (A)');
vtol_batt_V  = logsout_ssc_airvtol_elec.get('Battery Voltage (V)');
...
if(length(size(vtol_batt_C.Values.Data))>2)
    SOCres = squeeze(min(vtol_batt_C.Values.Data(:,1,:)));
else
    SOCres = vtol_batt_C.Values.Data;
end
plot(vtol_batt_C.Values.Time/60, SOCres, 'LineWidth', 1)
hold off
grid on
...
plot(vtol_batt_V.Values.Time/60, vtol_batt_V.Values.Data, 'LineWidth', 1)
grid on
title('Battery Voltage')
ylabel('Voltage (V)')

simlog_handles(3) = subplot(3, 1, 3);
plot(vtol_batt_A.Values.Time/60, vtol_batt_A.Values.Data, 'LineWidth', 1)
grid on
