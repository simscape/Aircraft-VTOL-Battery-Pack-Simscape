...
batt_5p12s_detailled_thr = Module(ParallelAssembly=pSet_thr, ...
    CoolingPlate = "Bottom",...
    NumSeriesAssemblies=12);

% Define grouping to control fidelity
batt_5p12s_detailled_thr.ModelResolution   = 'Detailed';


% Reuse figure if it exists for battery chart
if ~exist('h1_5p12s_detailed', 'var') || ...
        ~isgraphics(h1_5p12s_detailed, 'figure')
    h1_5p12s_detailed = figure('Name', 'h1_5p12s_detailed');
end
figure(h1_5p12s_detailed); clf(h1_5p12s_detailed)

% Create chart
ModuleChart = BatteryChart(Parent = h1_5p12s_detailed,...
    Battery=batt_5p12s_detailled_thr, SimulationStrategyVisible = "On");
xlabel('X');ylabel('Y'); 
...
batt_5p48s_detailled_thr = ModuleAssembly(...
    Module = repmat(batt_5p12s_detailled_thr,1,4),...
...
% Reuse figure if it exists for battery chart
if ~exist('h1_5p48s_detailled', 'var') || ...
        ~isgraphics(h1_5p48s_detailled, 'figure')
    h1_5p48s_detailled = figure('Name', 'h1_5p48s_detailled');
end
figure(h1_5p48s_detailled); clf(h1_5p48s_detailled)

% Create chart
BatteryChart(Parent = h1_5p48s_detailled,...
    Battery = batt_5p48s_detailled_thr, ...
...
batt_5p48s8p_detailled_thr = Pack(...
    ModuleAssembly=repmat(batt_5p48s_detailled_thr,1,8),...
...
% Reuse figure if it exists for battery chart
if ~exist('h1_5p48s8p_detailled', 'var') || ...
        ~isgraphics(h1_5p48s8p_detailled, 'figure')
    h1_5p48s8p_detailled= figure('Name', 'h1_5p48s8p_detailled');
end
figure(h1_5p48s8p_detailled); clf(h1_5p48s8p_detailled)

% Create chart
BatteryChart(Parent = h1_5p48s8p_detailled,...
    Battery = batt_5p48s8p_detailled_thr, ...
    SimulationStrategyVisible = "On");
xlabel('X');ylabel('Y'); 
...
if(~exist('+pack_5p48s8p_detailled_Thr','dir'))
    buildBattery(batt_5p48s8p_detailled_thr,...
        "LibraryName","pack_5p48s8p_detailled_Thr",...
